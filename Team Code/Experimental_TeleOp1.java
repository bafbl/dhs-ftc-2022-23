...
public class Experimental_TeleOp1 extends LinearOpMode
{
    private Experimental_TeamRobot robot;
    ...
    {
        robot = new Experimental_TeamRobot(this);
        ...
        while (!isStopRequested())
        {
            ...
            if ( gamepad1.left_bumper ) {
              // reset the heading if the left_bumper is pressed
              if ( gamepad1.dpad_up )
                ...
                // When no dpad is specified, round to the closest direction
                robot.resetHeadingToClosestWaypoint();
            } else {
              // left_bumper isn't pressed. Use dpad to set desired heading
              if ( gamepad1.dpad_up )
                robot.setDesiredHeading(0);
              else if (gamepad1.dpad_left )
                robot.setDesiredHeading(90);
              else if (gamepad1.dpad_down )
                robot.setDesiredHeading(180);
              else if (gamepad1.dpad_right )
                robot.setDesiredHeading(270);
            }
